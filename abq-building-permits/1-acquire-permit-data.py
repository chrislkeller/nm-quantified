...
class BuildingPermitRequest(object):
    ...
    request_url = 'http://data.cabq.gov/business/buildingpermits/BuildingPermitsCABQ-en-us.csv'

    downloaded_file = 'abq-building-permits.csv'
    ...
    output_csv = 'week-of-{0}-abq-weekly-commerical-permits.csv'.format(
        my_range.strftime('%Y-%m-%d')
    )

    output_file = 'week-of-{0}-abq-weekly-commerical-permits.txt'.format(
        my_range.strftime('%Y-%m-%d')
    )

    headline = 'Here are the commercial building permits issued in Albuquerque last week\n\n'
    ...
    def handle(self, *args, **kwargs):
        ...
        self.get_data(self.request_url, self.downloaded_file)

        # open the downloaded file
        with open(self.csv_name, newline='', encoding='utf-16') as file:
    ...
    def get_data(self, request_url, downloaded_file):
        ...
        urlretrieve(self.request_url, self.downloaded_file)

    def _process(self, data):
        logger.debug('processing the data')
        locale.setlocale(locale.LC_ALL, 'en_US.utf-8')
        for item in data:
            item_date = datetime.datetime(int(item['IssueDate'][0:4]), int(item['IssueDate'][4:6]), int(item['IssueDate'][6:]))
            item['proper_date'] = item_date
            if item_date > self.my_range and item['CommercialorResidential'] == 'Commercial':
                ...
                item['type_of_work'] = item['TypeofWork']
    ...
    def size_graf(self, item):
        output = ''
        output += 'The permit for the largest project in terms of square footage was issued on {permit_date} to {owner_name} for {TypeofWork} at {combined_address}.'.format(**item)
        if item['int_sq_feet'] > 0:
            output += ' The project is estimated to be about {int_sq_feet:,d} square feet.'.format(**item)
        if item['currency_valuation']:
            output += ' The estimated cost of the work is about {currency_valuation}. '.format(**item)
        output += 'The work will be done by {contractor_name}.\n\n'.format(**item)
        return output

    def cost_graf(self, item):
        output = ''
        output += 'The permit for the largest project in terms of estimated cost was issued on {permit_date} to {owner_name} for {TypeofWork} at {combined_address}.'.format(**item)
        if item['int_sq_feet'] > 0:
            output += ' The project is estimated to be about {int_sq_feet:,d} square feet.'.format(**item)
        if item['currency_valuation']:
            output += ' The estimated cost of the work is about {currency_valuation}. '.format(**item)
    ...
    def _write(self, largest_size, largest_value, sorted_data):
        ...
        for item in sorted_data:
            if item['ApplicationPermitNumber'] == largest_size['ApplicationPermitNumber'] or item['ApplicationPermitNumber'] == largest_value['ApplicationPermitNumber']:
                pass
            else:
                message = '{owner_name} received a permit on {permit_date} for {TypeofWork} at {combined_address}. The work will be done by {contractor_name}.'.format(**item)
                if item['int_sq_feet'] > 0:
                    message += ' The project is estimated to be about {int_sq_feet:,d} square feet.'.format(**item)
                if item['currency_valuation']:
                    message += ' The estimated cost of the work is about {currency_valuation}.'.format(**item)
                output += '\n• {}\n'.format(message)
        output += self.caveat
